...
export default function Home() {
  ...
  const { isPending, isError, error, data, isFetching, isPlaceholderData } =
    useQuery({
      ...
      queryFn: async () => await updatePokemons(),
    });

  const updatePokemons = async () => {
    const data = await fetch(
      `/api/pokemons?itemsPerPage=${MAX_ITEMS_ON_PAGE}&currentPage=${currentPage}&sort=${sort}`,
    ...
    <div className="mx-auto flex max-w-screen-md flex-col rounded bg-primary p-1 lg:max-w-screen-lg">
      ...
      <main className="flex flex-col">
        <div className="max-h-[calc(100dvh-170.44px)] min-h-[calc(100dvh-170.44px)] overflow-y-auto rounded-lg bg-grayscale-white px-3 py-6 shadow-inner_2dp lg:max-h-[calc(100dvh-176px)] lg:min-h-[calc(100dvh-176px)]">
          {isPending ? (
            <div className="flex h-[calc(100dvh-218.44px)] items-center justify-center">
              <div
            ...
            <PokemonCardList pokemon_list={data?.results} />
          )}
        </div>
        <Pagination
          page={currentPage}
          ...
          total_items={data ? data.amount_pokemons : 0}
        />
      </main>
    </div>
  );
